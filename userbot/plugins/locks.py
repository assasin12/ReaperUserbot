...
async def locks(event):
    ...
    if input_str == "msg":
        ...
        if not input_str:
            ...
            await event.edit(f"`Invalid lock type nigga. WTF are you doing:` {input_str}")
            return
    ...
    try:
        ...
        await event.edit(f"{DEFAULTUSER} `locked {what} Because go sleep idiot.!!`")
...
async def rem_locks(event):
    ...
    try:
        ...
        await event.edit(f"{DEFAULTUSER} `Unlocked {what} now Start Chit Chat but dont spam else you gey !!`")
    except BaseException as e:
        await event.edit(
            f"`Do I have proper rights for that ??`\n**Error:** {str(e)}")
        return
